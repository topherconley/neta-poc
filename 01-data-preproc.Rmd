...
```{r}
cna0 <- new.env()
load(envir = cna0, file = file.path(filterdir, "TCGA_OV169_CNA_170725.Rdata"))
```
...
keep_prot_index <- sd_pexp > quantile(x = sd_pexp, probs = 0.40)
protsdf <- vpocset[which(keep_prot_index),]
#no duplicated refseq ids
stopifnot(!anyDuplicated(featureNames(protsdf)))
```
...
```{r}
library(ggplot2)
qplot(mean_pexp, sd_pexp, geom = "point") + theme_bw() 
```
...
prot_node_attr_final <- cleaned_prot_attr[match(featureNames(protsdf), cleaned_prot_attr$refseq_peptide),]
...
featureData(protsdf) <- AnnotatedDataFrame(data = prot_node_attr_final, 
...
suppressPackageStartupMessages(library(Biobase))
prot_pco <- pcout(x = t(exprs(protsdf)))
...
stopifnot(sampleNames(protsdf) == sampleNames(cnaset))
protsdf <- protsdf[,!(sampleNames(protsdf) %in% protein_outliers)]
...
hc_protsdf <- hc_probes(protsdf)
library(data.table)
hcp <- rbindlist(lapply(hc_protsdf$hcp, function(x) data.frame(i = x[1], j = x[2])))
palias <- fData(protsdf)$alias
...
```{r}
#standardize after -> no outliers -> no multicolinearity
exprs(protsdf) <- t(scale(t(exprs(protsdf))))
saveRDS(object = protsdf, 
        file = file.path(filterdir, 
...
```{r}
set.seed(243)
drop_at_random <- hc_protsdf$hcp[hc_protsdf$dup]
