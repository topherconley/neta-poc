...
#Divide samples by collection site
...
cnaset <- cnaset[,tcga_pnnl_samples]
...
colnames(mpnnl) <- tcga_pnnl_samples
#subset of full to match the variable set
mpnnlv <- mpnnl[featureNames(protmad),]
protmad <- protmad[,tcga_pnnl_samples]
exprs(protmad) <- mpnnlv
```
...
cna_outliers <-  outlier_by_loc(cna_pco, 25)
```

We decided to remove the most extreme outliers, one from CNA and two from protein. 

```{r}
#assert sample names match
stopifnot(sampleNames(protmad) == sampleNames(cnaset))
protmad <- protmad[,!(sampleNames(protmad) %in% c(protein_outliers, cna_outliers))]
cnaset <- cnaset[,!(sampleNames(cnaset) %in%  c(protein_outliers, cna_outliers))]
stopifnot(sampleNames(protmad) == sampleNames(cnaset))
```
...
There is only a few pairs of highly correlated CNA that was reduced. 
