...
Import the ovarian microarray data and log2-transform the expression values. Then filter out expression values that are not in the top 12% of median absolute deviation (MAD) to have a consistent feature size for mRNA and protein data types. 

```{r}
library(readr)
rpoc <- read_csv(file = "~/scratch-data/neta-poc/mrna/mrna169_170807.csv")
rpoc_gene_symbols <- rpoc$X1
rpoc <- rpoc[,2:ncol(rpoc)]
colnames(rpoc)<-substr(colnames(rpoc), 1, 12)
red <- as.matrix(rpoc[,vpoc_samples])
red <- log2(red)
#convert gene symbol to RefseqID
rownames(red) <- rpoc_gene_symbols
#initial feature information
fd <-  data.frame(Symbol= rpoc_gene_symbols, 
                   row.names = rpoc_gene_symbols)
rpocset <- ExpressionSet(assayData = red,
                         featureData = AnnotatedDataFrame(data = fd))
rexp <- exprs(rpocset)
#no missing RNA
stopifnot(!any(is.na(rexp)))
sd_rexp <- apply(X = rexp, MARGIN = 1, FUN = sd)
mad_rexp <- apply(X = rexp, MARGIN = 1, FUN = mad)
mean_rexp <- apply(X = rexp, MARGIN = 1, FUN = mean)
keep_rna_index <- mad_rexp > quantile(x = mad_rexp, probs = 0.88)
rnamad <- rpocset[which(keep_rna_index),]
#no duplicated refseq ids
stopifnot(!anyDuplicated(featureNames(rnamad)))
rnamad
```

The mean-variance relationship of the microarray data is as follows. 

```{r}
qplot(mean_rexp, mad_rexp, geom = "point") + theme_bw() 
```

The standard deviation and the median absolute deviation are very correlated. 

```{r}
qplot(sd_rexp, mad_rexp, geom = "point") + theme_bw() 
```

Add annotation attributes for the mRNA features. 

```{r}
ensembl <- useMart("ensembl",dataset="hsapiens_gene_ensembl")
filters <- listFilters(ensembl)
grep("entrez", filters$name, value = T)
rna_hgnc_symbols  <- featureNames(rnamad)
rna_node_attr <- getBM(attributes = c('hgnc_symbol', 'entrezgene', 'chromosome_name',
                                      'start_position', 'end_position', 'band'),
                       filters = 'hgnc_symbol', 
                       values = rna_hgnc_symbols, 
                       mart = ensembl)
knitr::kable(head(rna_node_attr), row.names = F)
```

Handle duplicate HGSC symbols. 

```{r}
dupids <- rna_node_attr$hgnc_symbol[which(duplicated(rna_node_attr$hgnc_symbol))]
must_dedup_index <- rna_node_attr$hgnc_symbol %in% dupids
must_dedup <- rna_node_attr[must_dedup_index,]
must_dedup_group <- split(x = must_dedup, 
                          f = as.factor(must_dedup$hgnc_symbol))
suppressPackageStartupMessages(library(data.table))
fclean_dups <- function(i) { 
  j <- grep("CHR_", must_dedup_group[[i]]$chromosome_name, invert = T)  
    #HLA locus
...
#missing attributes for some features
miss_rna_attr <- data.frame(hgnc_symbol= setdiff(rna_hgnc_symbols, rna_node_attr$hgnc_symbol),
                             entrezgene = NA, 
                             chromosome_name = NA, 
                             start_position = NA, 
                             end_position = NA, 
...
#combine non-duplicated, de-duplicated, missing attributes
cleaned_rna_attr <- rbind(rna_node_attr[!must_dedup_index,], clean_dups, miss_rna_attr)
#View(cleaned_rna_attr)
#match the correct order as the eset feature names
rna_node_attr_final <- cleaned_rna_attr[match(featureNames(rnamad), cleaned_rna_attr$hgnc_symbol),]
#View(rna_node_attr_final)
metaData <- data.frame(labelDescription= colnames(rna_node_attr_final))
colnames(rna_node_attr_final) <- c("id", "alias", "chr", "start", "end", "cytoband")
featureData(rnamad) <- AnnotatedDataFrame(data = rna_node_attr_final, 
                                          varMetadata = metaData)
```
...
rna_pco <-  pcout(x = t(exprs(rnamad)))
```
...
plot_outliers(rna_pco) + ggtitle("mRNA Outlier Analysis")
```
...
rna_outliers <- outlier_by_loc(rna_pco, 30)
cna_outliers <-  outlier_by_loc(cna_pco, 19.5)
#no comon intersection
#intersect(protein_outliers, rna_outliers)
```

We analyzed protein before mRNA early in the study, and at that point in time we decided to remove the most extreme outliers from protein. We postulate subtle differences in training/test sets between mRNA and protein---induced by removing the mRNA outlying samples---could have more undesirable outcome than removing a few outlying samples specific to mRNA. Therefore, we will not remove any outliers specific to mRNA and proceed. 

```{r}
#assert sample names match
stopifnot(sampleNames(protmad) == sampleNames(cnaset), sampleNames(rnamad) == sampleNames(cnaset))
protmad <- protmad[,!(sampleNames(protmad) %in% protein_outliers)]
rnamad <- rnamad[,!(sampleNames(rnamad) %in% protein_outliers)]
cnaset <- cnaset[,!(sampleNames(cnaset) %in% protein_outliers)]
stopifnot(sampleNames(protmad) == sampleNames(cnaset))
stopifnot(sampleNames(rnamad) == sampleNames(cnaset))
```
...
### Highly Correlated mRNA

We clustered the microarray probes in the same fashion as the protein and CNA features (see below). No probes were found to be highly correlated. 

```{r}
hc_mrna <- function(eset) { 
  X <- t(exprs(eset))
  library(fpc)
  dR <- 1 - abs(cor(X))
  clout <- fpc::dbscan(data = dR, MinPts = 2, eps = 1e-3, method = "dist")
  ...
  clusterDistr <- as.data.frame(table(clout$cluster), stringAsFactors = FALSE)
  #nothing to cluster
  if(nrow(clusterDistr)) { 
    exprs(eset) <- t(scale(t(exprs(eset))))
    saveRDS(object = eset, 
            file = file.path(filterdir, 
                             "poc-mrna-eset-dropout-std.rds"))
    return(eset)
  }
  #Cluster `0` corresponds to sufficiently independent CNA's.
  #Omit it from the multi-colinear analysis.
