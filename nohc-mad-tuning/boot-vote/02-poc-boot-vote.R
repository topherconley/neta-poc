...
#result directory
respath <- "/home/cconley/scratch-data/neta-poc/nohc-mad-boot-vote/02"
...
tictoc <- system.time({ens <- spacemap::bootEnsemble(Y = Y, X = X, tune = tune,
                                                     ...
                                                     seed = seed, p0 = 0.90,
                                                     tol = 1e-4, cdmax = 90e7)})
save.image(file = file.path(respath, "poc-boot-vote-p90-B1000.rda"))
...
#  SAVE RESULTS  #
##################
saveRDS(object = bv, file = file.path(respath, "poc-boot-vote-p90-B1000.rds"))
save.image(file = file.path(respath, "poc-boot-vote-p90-B1000.rda"))
